                ...
                <div class="card-header bg-white pt-3 pb-0 border-bottom-0">
                    <ul class="nav nav-tabs" id="chatTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="user-tab" data-bs-toggle="tab" data-bs-target="#user-pane" type="button" role="tab">User</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active fw-bold" id="leader-tab" data-bs-toggle="tab" data-bs-target="#leader-pane" type="button" role="tab">Team Leader</button>
                        </li>
                    </ul>
                </div>

                <div class="tab-content flex-grow-1 d-flex flex-column" id="myTabContent">
                    <div class="tab-pane fade" id="user-pane" role="tabpanel" tabindex="0">
                        <div class="p-4 text-center text-muted">User channel is currently quiet.</div>
                    </div>

                    <div class="tab-pane fade show active flex-grow-1 d-flex flex-column" id="leader-pane" role="tabpanel" tabindex="0">
                        <div class="chat-box">
                            <div class="chat-sender ms-1">Team Leader</div>
                            <div class="chat-bubble other">Theme is set for SME. Let's define user roles.</div>

                            <div class="chat-sender ms-1 mt-2">User</div>
                            <div class="chat-bubble other">We need distinct access for admins, users, developers.</div>

                            <div class="chat-sender ms-1 mt-2">Kisu (Infra Lead)</div>
                            <div class="chat-bubble other">Can web team make mockup? We'll build security around it.</div>

                            <div class="chat-sender me me-1 mt-3">Me (Web Lead)</div>
                            <div class="chat-bubble me">Aulim Security Portal Mockup. Here are the pages.</div>
                        </div>

                        <div class="card-footer bg-white p-3">
                            <div class="input-group">
                                <input type="text" class="form-control bg-light border-0" placeholder="Type a message...">
                                <button class="btn btn-primary px-4" type="button">Send <i class="fas fa-paper-plane ms-1"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function handleAuth() {
            if (localStorage.getItem('token')) {
                ...
                alert('로그아웃 되었습니다.');
                window.location.href = 'login.php'; // .php로 수정
            } else {
                window.location.href = 'login.php'; // .php로 수정
            }
        }
